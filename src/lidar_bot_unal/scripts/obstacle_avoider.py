...
class ObstacleAvoiderNode(Node):
    ...
    def scan_cb(self, msg: LaserScan):
        ...
        # --- ADDED ROBUSTNESS to prevent crashes with min() on empty lists ---
        ...
        if obstacle_detected:
            self.get_logger().info(f"Obstacle detected! Overriding goal.", throttle_duration_sec=1.0)
        else:
            final_cmd = self.goal_cmd

        # Smooth the final command before publishing
        alpha = 0.5
        smooth_cmd = Twist()
